...
try:
    from ..ops.selective_scan_interface import (
        bimamba_inner_fn,
        mamba_inner_fn,
...
try:
    from ..ops.triton.selective_state_update import (
...
try:
    from ..ops.triton.layernorm import (
        layer_norm_fn,
        rms_norm_fn,
        RMSNorm,
    )
...
class Mamba(nn.Module):
    def __init__(
        ...
        #print("here", self.dt_proj.bias.ds_tensor.shape, inv_dt.shape)
        #print(f"Rank {torch.distributed.get_rank()}, ds_tensor shape: {self.dt_proj.bias.ds_tensor.shape}  inv_dt shape: {inv_dt.shape}")
        if hasattr(self.dt_proj.bias, "ds_tensor"):
            # 1 GPU
            # with torch.no_grad():
            #     self.dt_proj.bias.ds_tensor.copy_(inv_dt)
            
            # Multi-GPUs
            from deepspeed.runtime.zero.partition_parameters import GatheredParameters
            with GatheredParameters(self.dt_proj.bias, modifier_rank=0):
                if torch.distributed.get_rank() == 0:  # Only update on rank 0
                    self.dt_proj.bias.data.copy_(inv_dt.to(self.dt_proj.bias.dtype))
